...
export class SceneryModel {
  ...
  observe(cameraId: string, detections: readonly Detection[], nowMs: number): void {
    const existing = (this.tracks.get(cameraId) ?? []).filter(
      (t) => nowMs - t.lastMs < SCENERY_FORGET_MS,
    );
  ...
  isScenery(cameraId: string, detection: Detection, nowMs: number): boolean {
    ...
    for (const track of this.tracks.get(cameraId) ?? []) {
      ...
      if (nowMs - track.lastMs >= SCENERY_FORGET_MS) continue;
      if (iou(track.box, detection.box) < SCENERY_IOU) continue;
      const samples = track.seen + track.missed;
      if (track.lastMs - track.firstMs < SCENERY_MIN_AGE_MS) continue;
      if (samples < SCENERY_MIN_SAMPLES) continue;
      if (track.seen / samples < SCENERY_MIN_PRESENCE) continue;
      return true;
    }
    return false;
  }
  ...
  describe(cameraId: string, nowMs: number): string[] {
    return (this.tracks.get(cameraId) ?? [])
      .filter((t) => {
        const samples = t.seen + t.missed;
        return (
          t.lastMs - t.firstMs >= SCENERY_MIN_AGE_MS &&
          samples >= SCENERY_MIN_SAMPLES &&
          t.seen / samples >= SCENERY_MIN_PRESENCE &&
          nowMs - t.lastMs < SCENERY_FORGET_MS
        );
      })
      .map(
        (t) =>
          `${t.kind} at [${t.box.map((v) => v.toFixed(2)).join(",")}] ` +
          `(${t.seen}/${t.seen + t.missed} over ${Math.round((t.lastMs - t.firstMs) / 60_000)}min)`,
      );
  }
}
