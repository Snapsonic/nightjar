...
Settings for [Google Drive backup](/guides/recording-and-events#google-drive-backup). Normally toggled from the dashboard's "Google Drive backup" card (or the local UI); the block is additive — existing configs without it keep parsing.

| Field | Type | Default | Notes |
| --- | --- | --- | --- |
| `enabled` | boolean | `false` | Copy newly closed events to your Drive |
| `folderName` | string (1–80) | `"Nightjar"` | Top-level Drive folder clips are filed under |
| `shareLinks` | boolean | `false` | Give each newly backed-up clip an "anyone with the link" reader permission |
| `clientId` | string | — | Your own Google OAuth "TVs and Limited Input devices" client |
| `clientSecret` | string | — | Its secret |

Client credentials resolve per field: environment variable, then this block, then the client baked into the build (official images only). The OAuth tokens themselves are **not** in `config.json` — they live in `${CONFIG_DIR}/gdrive-token.json` (mode 0600) and never leave the node.

### `nest`

Credentials for the experimental [Nest camera bridge](/guides/cameras#nest-cameras-experimental-sdm-bridge) (Google Smart Device Management). All optional; normally filled in from the local UI's "Nest cameras" card. The block is additive — existing configs without it keep parsing.

| Field | Type | Default | Notes |
| --- | --- | --- | --- |
| `projectId` | string | — | Device Access project ID (console.nest.google.com/device-access) |
| `clientId` | string | — | Your own Google OAuth "Web application" client |
| `clientSecret` | string | — | Its secret |

The OAuth refresh token itself is **not** in `config.json` — it lives in `${CONFIG_DIR}/nest-token.json` (mode 0600), written when you complete the connect flow in the UI.
...
| `source` | `"rtsp"` \| `"nest"` | `"rtsp"` | Where the video comes from. Existing configs (no field) parse as `"rtsp"` |
| `rtspUrl` | URL | — | Main stream. Required when `source` is `"rtsp"`. May embed credentials — never leaves the node |
| `rtspSubUrl` | URL | — | Optional substream, used for motion detection and low-bandwidth live view (RTSP cameras only) |
| `nest.deviceId` | string | — | SDM device id. Required when `source` is `"nest"` |
...
| `NIGHTJAR_GOOGLE_CLIENT_ID` | — | OAuth client for [Google Drive backup](/guides/recording-and-events#google-drive-backup). Overrides `backup.gdrive.clientId` |
| `NIGHTJAR_GOOGLE_CLIENT_SECRET` | — | Its secret. Overrides `backup.gdrive.clientSecret` |
