...
Deletion is permanent and there is no cloud trash. If you want clips to outlive the retention window, either keep them local (the node's own recordings are never touched by this) or turn on [Google Drive backup](/guides/recording-and-events#google-drive-backup), which copies clips into storage you control and which Nightjar never expires.
